...
Input_File* Linker::read_input_file(string path) {
    ...
    if (in_file.is_open()) {
        ...
        auto data = new Elf16_Byte[result->header.shentries * sizeof(Elf16_SH_Entry)];
        ...
        for (auto& section : result->section_headers) {
            auto data = new Elf16_Byte[section.size];
            in_file.seekg(section.offs);
            in_file.read((char*)data, section.size);
            result->sections.push_back(vector<Elf16_Byte>(data, data+section.size));
            delete[] data;
        }
    } else {
        cout << "Error";
